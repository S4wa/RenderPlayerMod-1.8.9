package net.simplyrin.renderplayermod.utilities;
...
public class GuiSlideControl extends GuiButton {
	...
	public GuiSlideControl(int id, int x, int y, int width, int height, String displayString, float minVal,
			float maxVal, float curVal, boolean useInts) {
		super(id, x, y, width, height,
				(useInts ? displayString + ((int) curVal) : displayString + numFormat.format(curVal)));
	...
	protected void mouseDragged(Minecraft mc, int mousePosX, int mousePosY) {
		if (visible) {
			...
			this.drawTexturedModalRect(xPosition + (int) (curValue * (float) (width - 8)) + 4, yPosition, 196, 66, 4,
					20);
		}
	}
